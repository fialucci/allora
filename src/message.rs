...
//! Basic creation:
//! ```
//! use allora::message::{Exchange , Message};
//! let msg = Message::from_text("hello");
//! assert!(msg.header("message_id").is_some());
//! let mut exch = Exchange::new(msg);
//! // lazily create correlation id
//! let cid = exch.correlation_id().to_string();
//! assert_eq!(exch.in_msg.header("correlation_id"), Some(cid.as_str()));
//! ```
//!
//! With bytes payload:
//! ```
//! use allora::message::{Message, Payload};
//! let msg = Message::new(Payload::Bytes(vec![1,2,3]));
//! assert_eq!(msg.payload.as_bytes(), Some(&[1,2,3][..]));
//! ```
//!
//! Adding and overriding headers:
//! ```
//! use allora::message::Message;
//! let mut msg = Message::from_text("data");
//! msg.set_header("content-type", "text/plain");
//! msg.set_header("content-type", "text/custom");
//! assert_eq!(msg.header("content-type"), Some("text/custom"));
//! ```
